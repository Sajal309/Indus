...
$user_details = $_SESSION['user_details'];
    ...
    curl_setopt_array($curl,array(
       ...
       CURLOPT_URL => $url . "products/item/" . $prd_id
       ));
    ...
    if(!empty($_POST['quantity'])){
        if(!empty($user_details['user_id'])){
            ...
            $_SESSION['status'] = "product_details";

        ?>
        <script> window.location = 'login.php?product_id=<?php echo $prd_id;?>&group_id=<?php echo $grp_id;?>'; </script>
        <?php  }
    }
    $run = null;
    do{
    ...
    $cart_res = json_decode($response3,true);
?>

<?php

if($cart_res['status'] == "success"){
  echo "product added to cart";
  echo $cart_res['message'];
}else if($cart_res['status'] == "failed"){
...
<body>
    ...
    <div id="main" >
        ...
        <?php $_SESSION['prd_id'] = $value['images'][0]['prd_id'];
              $_SESSION['grp_id'] = $value['grp_id'];?>
        <div id="list_image" >
            <div id="img_box">
        ...
        <div id="main_image" >
             <div id="image" >
                    <img src="<?php echo $media . $value['images'][0]['image']; }?>" alt="alt" >
            </div> 
        </div>
                    ...
                    <div id="additional">
                            <div id="size">
                                ...
                                <?php foreach($value['size'] as $k => $v){ ?>
                                    <button id="size_p" name="size" type="button" value="<?php echo $v['size'];?>"><?php echo $v['size']; ?></button>
                                <?php } ?>
                                 </div>
                            </div>
                            <div id="color">
                                    ...
                                    <?php foreach($value['colors'] as $k => $v){ ?>
                                        <button id="color_p" name="color" style="background: <?php echo $v['color']; ?>" type="button" value="<?php echo $v['color'];?>"></button>
                                        <?php } ?>
                                    </div>
                            </div>
                            <div id="color">
                                    <p id="description" style="margin-top:10px;">Choose Quantity</p>
                                <input type="number" placeholder="Quantity" name="quantity" style="width:100%;height:25px;padding:5px;background: white;border: 1px solid rgb(0,0,0,0.4);">
                            </div>
                    </div>
                    <div id="actions">   
                          
                            <button id="addtocart" type="submit">ADD TO BAG   <i class="material-icons">local_mall </i></button>
                    </div>
            </form>
                    <div id="more_details">
